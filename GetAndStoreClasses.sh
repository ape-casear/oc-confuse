...
# 需要过滤的文件夹和文件
filterConfig="$(pwd)/configures/filterDirAndFileOfClass.cfg"
...
post_implement_file_handle() {
	...
	do
		...
		if [[ ${#line} -le 6 ]] || [[ $(expr "$line" : '^#.*') -gt 0 ]] || [[ ${line} =~ ${category_judge_substring} ]]; then
	...
	# 添加最后一个类
	if [[ ${#lastLine} -le 6 ]] || [[ $(expr "$lastLine" : '^#.*') -gt 0 ]] || [[ ${lastLine} =~ ${category_judge_substring} ]]; then
		# 长度小于等于6、注释内容的行、在黑名单中的内容、分类文件不处理

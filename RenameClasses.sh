...
# class_search_dir="$(pwd)/Sup_SDK_ket"
...
# 定义该目录下的文件需要进行批量替换处理
class_name_replace_dir=$class_search_dir
...
function add_prefix_suffix_to_class {
	...
	for (( i = 0; i < ${#config_content_array[@]}; i++ )); do
		...
		result_class_name=$(sed -n "$(($RANDOM%9900))p" ./words)_$(sed -n "$(($RANDOM*2))p" ./words)_$(sed -n "$(($RANDOM*2))p" ./words);
		if [[ $(expr $original_class_name : $project_name) -gt 0 ]]
		then 
			echo -e "\033[31m PROJECT_NAME is ${result_class_name} \033[0m"
			echo ${result_class_name} > 'projectName.txt'
			echo ${project_name} > 'originProjectName.txt'
		fi
		...
		sed -i '{
			s/'"${original_class_name}"'/'"${result_class_name}"'/g
		}' `grep ${original_class_name} -rl ${class_name_replace_dir} | sed -n '/xcodeproj/ !p'`
...
#		new_file_path=${find_result/$original_class_name/$result_class_name}
		new_file_path=$(echo "${find_result}" | sed -r "s/(.*)${original_class_name}/\1${result_class_name}/")

...
####### 配置
# classes类目录
property_name_replace_dir="$(pwd)/Heywood_Wall_acclamation/Heywood_Wall_acclamation"
# 黑名单类目录
...
function rename_properties {
	...
	for (( i = 0; i < ${#rename_properties_config_content_array[@]}; i++ )); do
		...
		result_prop_name=$(sed -n "$(($RANDOM))p" ./words)_$(sed -n "$(($RANDOM*2))p" ./words)_$(sed -n "$(($RANDOM*2))p" ./words);
		sed -i -r '{
			s/([^_])'"${original_prop_name}"'/\1'"${result_prop_name}"'/g
